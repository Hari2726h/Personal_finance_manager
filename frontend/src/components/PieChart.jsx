...
const PieChart = ({ data }) => {
  ...
  const options = {
    ...
    plugins: {
      legend: { labels: { color: darkMode ? '#fff' : '#000', boxWidth: 10 } },
    },
  ...
  const ChartComp = chartType === 'bar' ? Bar : chartType === 'doughnut' ? Doughnut : Pie;
  ...
  return (
    <div className={`card p-2 ${darkMode ? 'bg-dark text-white' : ''}`} style={{ maxWidth: 400, margin: 'auto' }}>
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-2 gap-2">
        ...
        <button className="btn btn-sm btn-outline-success" onClick={exportAsImage}>PNG</button>
        <button className="btn btn-sm btn-outline-danger" onClick={exportAsPDF}>PDF</button>
      </div>
